...
const PROGMEM PS2Keymap_t PS2Keymap_US = {
  ...
  // control
	{0, PS2_F9, 0, PS2_F5, PS2_F3, PS2_F1, PS2_F2, PS2_F12,
	0, PS2_F10, PS2_F8, PS2_F6, PS2_F4, PS2_TAB, '^', 0,
	0, 0 /*Lalt*/, 0 /*Lshift*/, 0, 0 /*Lctrl*/, 0x11, '1', 0,
	0, 0, 0x19, 0x13, 0x01, 0x17, 0x00 /* NUL / @, should be ctrl-shift-2 */, 0,
	0, 0x03, 0x18, 0x04, 0x05, '4', '3', 0,
	0, ' ', 0x16, 0x06, 0x14, 0x12, '5', 0,
	0, 0x0e, 0x02, 0x08, 0x07, 0x1a, 0x1e /* should be ctrl-shift-6 */, 0,
	0, 0, 0x0d, 0x0a, 0x15, '7', '8', 0,
	0, ',', 0x0b, 0x09, 0x0f, '0', '9', 0,
	0, '.', '-', 0x0c, PS2_o_DIAERESIS, 0x10, 0x1f /* should be ctrl-shift-(-) for _ */, 0,
	0, 0, PS2_a_DIAERESIS, 0, 0x1b, '\'', 0, 0,
	0 /*CapsLock*/, 0 /*Rshift*/, PS2_ENTER /*Enter*/, 0x1d, 0, 0x1c, 0, 0,
...
#define SHIFT_R   0x08
#define ALTGR     0x10
#define CONTROL_L 0x20
// Note: CONTROL_R is scancode 0xE0 0x14, and it's not yet being tracked
...
static char get_iso8859_code(void)
{
	...
	while (1) {
		...
		if (s == 0xF0) {
			...
			} else if (state & (CONTROL_L | CONTROL_R)) {
				if (s < PS2_KEYMAP_SIZE)
					c = pgm_read_byte(keymap->control + s);
			} else {
				if (s < PS2_KEYMAP_SIZE)
